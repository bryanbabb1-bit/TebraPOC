...
def get_box_client():
    ...
    print(f"Connecting to Box Enterprise: {enterprise_id} via Tebra_Data_Bridge...")
    auth = CCGAuth(
        client_id=client_id,
...
def get_latest_file_id(client, folder_id, name_prefix):
    """Finds the newest file in a folder based on content modification time."""
    print(f"Scanning folder {folder_id} for newest '{name_prefix}' reports...")
    ...
    if latest_item:
        print(f"  > Selected: {latest_item.name} (ID: {latest_item.id})")
        return latest_item.id
    return None

def congregate_data():
    print("--- Starting PractiSynergy POC Data Sync ---")
    try:
        ...
        if not (claims_file_id and revenue_file_id):
            print("ERROR: Missing files. Ensure files start with prefixes and App is a collaborator.")
            return

        # 2. Download files
        print("Downloading files from Box...")
        ...
        print(f"SUCCESS: Dashboard data updated via Tebra_Data_Bridge.")
